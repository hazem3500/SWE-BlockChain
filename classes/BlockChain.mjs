...
export default class BlockChain {
    ...
    async minePendingTransactions(miningRewardAddress) {
        ...
        if (!this.pendingTransactions.length) {
            console.log(
                `${emojic.warning}  ${colorIt(
                    'There are no pending transactions for you to mine.'
                ).wetAsphalt()}`
            );
            return;
        }
    ...
    addTransaction(transaction, publicKey) {
        // fill the array of transactions in the blockChain  //Mo3
        if (!transaction.fromAddress || !transaction.toAddress) {
            console.log(
                `${emojic.noEntry}  ${colorIt(
                    'Transaction must contain from and to address'
                ).red()}`
            );
            return;
        }

        if (transaction.amount > this.getBalanceOfAddress(publicKey)) {
            console.log(
                `${emojic.noEntry}  ${colorIt(
                    `You don't have enough ${emojic.potato}  coins!`
                ).red()}`
            );
            return;
        }
        ...
        if (!transaction.isValid()) {
            console.log(
                `${emojic.noEntry}  ${colorIt(
                    "Can't add invalid transaction to the chain."
                ).red()}`
            );
            return;
        }
